...
export const Sidebar:FC<SideBarProps> = ({ toggleOpen, sidebarIsOpen }) => {
  ...
  const handleClose = () => toggleOpen(sidebarIsOpen? false: true);

  const renderAccountBalance = useCallback(
    () => <p className="Sidebar__account-money item--text">100000</p>,
    []
  );
  ...
  const renderCabinet = useCallback(
    () => <p className='Sidebar__account-cabinet item--text'>Кабинет</p>,
    []
  )
  const renderEnergy = useCallback(
    () => <p className="Sidebar__account-energy item--text">100000</p>,
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  );
